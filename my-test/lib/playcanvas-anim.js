Object.assign(
  ...
  (function () {
    ...
    AnimationKeyable._cloneValue = function (value) {
      if (value instanceof pc.Vec3 || value instanceof pc.Quat) return value.clone();
      // just in case there's something not covered by pc.Vec3 and pc.Quat
      if (value !== undefined && typeof value.clone === "function") return value.clone();
    ...
    AnimationKeyable.sum = function (keyable1, keyable2) {
      if (!keyable1 || !keyable2 || keyable1.type != keyable2.type) return null;

      var resKeyable = new AnimationKeyable(keyable1.type);
      switch (keyable1.type) {
        case AnimationKeyableType.NUM:
          resKeyable.value = keyable1.value + keyable2.value;
          break;
        case AnimationKeyableType.VEC:
          resKeyable.value.add2(keyable1.value, keyable2.value);
          break;
        case AnimationKeyableType.QUAT:
          ...
          resKeyable.value.w = keyable1.value.w + keyable2.value.w;
          break;
      }
      return resKeyable;
    };
    ...
    AnimationKeyable.minus = function (keyable1, keyable2) {
      ...
      switch (keyable1.type) {
        case AnimationKeyableType.NUM:
          ...
          break;
        case AnimationKeyableType.VEC:
          resKeyable.value.sub2(keyable1.value, keyable2.value);
          break;
        case AnimationKeyableType.QUAT:
          resKeyable.value.x = keyable1.value.x - keyable2.value.x;
          resKeyable.value.y = keyable1.value.y - keyable2.value.y;
    ...
    AnimationKeyable.mul = function (keyable, coeff) {
      ...
      switch (keyable.type) {
        case AnimationKeyableType.NUM:
          resKeyable.value *= coeff;
          break;
        case AnimationKeyableType.VEC:
          resKeyable.value.scale(coeff);
          break;
        case AnimationKeyableType.QUAT:
          resKeyable.value.x *= coeff;
          resKeyable.value.y *= coeff;
          resKeyable.value.z *= coeff;
          resKeyable.value.w *= coeff;
          break;
      }
      return resKeyable;
    };
    ...
    AnimationKeyable.linearBlend = function (keyable1, keyable2, p, cacheValue) {
      ...
      switch (keyable1.type) {
        case AnimationKeyableType.NUM:
          resKeyable.value = (1 - p) * keyable1.value + p * keyable2.value;
          break;
        case AnimationKeyableType.VEC:
          resKeyable.value.lerp(keyable1.value, keyable2.value, p);
          break;
        case AnimationKeyableType.QUAT:
          resKeyable.value.slerp(keyable1.value, keyable2.value, p);
          break;
      }
      return resKeyable;
    };
    ...
    AnimationTarget.prototype.toString = function () {
      var str = "";
      if (this.targetNode) str += this.targetNode.name;
      if (this.targetPath) str += "_" + this.targetPath;
      if (this.targetProp) str += "_" + this.targetProp;
      return str;
    };
    ...
    AnimationTarget.prototype.updateToTarget = function (value) {
      ...
      if (this.targetNode && this.targetNode[this.targetPath] !== undefined) {
        ...
        else this.targetNode[this.targetPath] = value;
    ...
    Object.defineProperty(AnimationCurve.prototype, "isPlaying", {
      get: function () {
        if (this.session) return this.session.isPlaying;
        return false;
      },
      set: function (isPlaying) {
        if (this.session) this.session.isPlaying = isPlaying;
      },
    });
    Object.defineProperty(AnimationCurve.prototype, "loop", {
      get: function () {
        if (this.session) return this.session.loop;
        return false;
      },
      set: function (loop) {
        if (this.session) this.session.loop = loop;
      },
    });
    Object.defineProperty(AnimationCurve.prototype, "bySpeed", {
      get: function () {
        if (this.session) return this.session.bySpeed;
        return 0;
      },
      set: function (bySpeed) {
        if (this.session) this.session.bySpeed = bySpeed;
      },
    });
    ...
    AnimationCurve.prototype.blendToTarget = function (keyable, p) {
      if (this.session) this.session.blendToTarget(keyable, p);
    };
    ...
    AnimationCurve.prototype.updateToTarget = function (keyable) {
      if (this.session) this.session.updateToTarget(keyable);
    };
    ...
    AnimationCurve.prototype.on = function (name, time, fnCallback, context, parameter) {
      if (this.session) this.session.on(name, time, fnCallback, context, parameter);
      return this;
    };
    ...
    AnimationCurve.prototype.off = function (name, time, fnCallback) {
      if (this.session) this.session.off(name, time, fnCallback);
      return this;
    };
    ...
    AnimationCurve.prototype.removeAllEvents = function () {
      if (this.session) this.session.removeAllEvents();
      return this;
    };
    ...
    AnimationCurve.prototype.fadeIn = function (duration) {
      if (!this.session) this.session = new AnimationSession(this);
    ...
    AnimationCurve.prototype.fadeOut = function (duration) {
      if (this.session) this.session.fadeOut(duration);
    };
    ...
    AnimationCurve.prototype.showAt = function (time, fadeDir, fadeBegTime, fadeEndTime, fadeTime) {
      if (this.session) this.session.showAt(time, fadeDir, fadeBegTime, fadeEndTime, fadeTime);
    };
    ...
    AnimationCurve.prototype.insertKey = function (type, time, value) {
      if (this.keyableType != type) return;

      var pos = 0;
      while (pos < this.animKeys.length && this.animKeys[pos].time < time) pos++;
    ...
    AnimationCurve.prototype.insertKeyable = function (keyable) {
      if (!keyable || this.keyableType != keyable.type) return;
    ...
    AnimationCurve.prototype.shiftKeyTime = function (time) {
      for (var i = 0; i < this.animKeys.length; i++) this.animKeys[i].time += time;
    };
    ...
    AnimationCurve.prototype.getSubCurve = function (tmBeg, tmEnd) {
      ...
      for (i = 0; i < this.animKeys.length; i++) {
        if (this.animKeys[i].time >= tmBeg && this.animKeys[i].time <= tmEnd) {
          if (tmFirst < 0) tmFirst = this.animKeys[i].time;
    ...
    AnimationCurve.prototype.evalLINEAR = function (time) {
      if (!this.animKeys || this.animKeys.length === 0) return null;
    ...
    AnimationCurve.prototype.evalLINEAR_cache = function (time, cacheKeyIdx, cacheValue) {
      ...
      for (var c = 0; c < this.animKeys.length; c++) {
        ...
        if (this.animKeys[i].time > time && (i - 1 < 0 || this.animKeys[i - 1].time < time)) {
    ...
    AnimationCurve.prototype.evalSTEP = function (time) {
      ...
      for (var i = 1; i < this.animKeys.length; i++) {
        if (this.animKeys[i].time <= time) key = this.animKeys[i];
        else break;
      }
      var resKey = new AnimationKeyable();
      resKey.copy(key);
    ...
    AnimationCurve.prototype.evalCUBICSPLINE_GLTF = function (time) {
      ...
      if (this.keyableType === AnimationKeyableType.NUM) {
        resKey.value = AnimationCurve.cubicHermite(
          g * key1.outTangent,
          key1.value,
          g * key2.inTangent,
          key2.value,
        ...
        resKey.value.x = AnimationCurve.cubicHermite(
          g * key1.outTangent.x,
          key1.value.x,
          g * key2.inTangent.x,
          key2.value.x,
        ...
        resKey.value.y = AnimationCurve.cubicHermite(
          g * key1.outTangent.y,
          key1.value.y,
          g * key2.inTangent.y,
          key2.value.y,
        ...
        resKey.value.z = AnimationCurve.cubicHermite(
          g * key1.outTangent.z,
          key1.value.z,
          g * key2.inTangent.z,
          key2.value.z,
        ...
        resKey.value.w = AnimationCurve.cubicHermite(
          g * key1.outTangent.w,
          key1.value.w,
          g * key2.inTangent.w,
          key2.value.w,
    ...
    AnimationCurve.prototype.eval = function (time) {
      ...
      switch (this.type) {
        ...
        case AnimationCurveType.CUBIC:
          if (this.keyableType == AnimationKeyableType.QUAT) return this.evalLINEAR(time);
    ...
    AnimationCurve.cubicCardinal = function (key0, key1, key2, key3, time, tension, cacheValue) {
      ...
      if (!key1 || !key2 || key1.type != key2.type) return null;
      ...
      if (key1.type === AnimationKeyableType.NUM) {
        ...
        if (key0) m1 = (2 * factor * (key2.value - key0.value)) / (key2.time - key0.time);

        m2 = (factor * (key2.value - key1.value)) / (key2.time - key1.time);
        if (key3) m2 = (2 * factor * (key3.value - key1.value)) / (key3.time - key1.time);
      ...
      if (key1.type === AnimationKeyableType.VEC) {
        ...
        for (var i = 0; i < props.length; i++) {
          var pr = props[i];
          if (resKey.value[pr] === undefined) continue;
          m1 = (factor * (key2.value[pr] - key1.value[pr])) / (key2.time - key1.time);
          if (key0) m1 = (2 * factor * (key2.value[pr] - key0.value[pr])) / (key2.time - key0.time);

          m2 = (factor * (key2.value[pr] - key1.value[pr])) / (key2.time - key1.time);
          if (key3) m2 = (2 * factor * (key3.value[pr] - key1.value[pr])) / (key3.time - key1.time);
    ...
    AnimationClipSnapshot.linearBlend = function (shot1, shot2, p) {
      ...
      for (var i = 0; i < shot2.curveNames.length; i++) {
        ...
        if (shot1.curveKeyable[cname] && shot2.curveKeyable[cname]) {
          var resKey = AnimationKeyable.linearBlend(
        ...
        else if (shot2.curveKeyable[cname]) resShot.curveKeyable[cname] = shot2.curveKeyable[cname];
      }
      return resShot;
    };
    ...
    AnimationClip.prototype.addCurve = function (curve) {
      if (curve && curve.name) {
        ...
        if (curve.duration > this.duration) this.duration = curve.duration;
      }
    };
    ...
    AnimationClip.prototype.eval_cache = function (time, cacheKeyIdx, cacheValue) {
      // 1226
      if (!cacheValue) {
        var ret = this.eval();
    ...
    */
    ...
    AnimationClip.prototype.transferToRoot = function (root) {
      ...
      for (var i = 0, len = this.animCurves.length; i < len; i++) {
        ...
        if (curve.animTargets[0].targetNode === "model") curve.animTargets[0].targetNode = root;
    ...
    AnimationClip.prototype.updateCurveNameFromTarget = function () {
      for (var i = 0, len = this.animCurves.length; i < len; i++) {
        var curve = this.animCurves[i];
        if (!curve.animTargets || curve.animTargets.length < 1) continue;
    ...
    */
    ...
    var AnimationSession = function AnimationSession(playable, targets) {
      ...
      if (playable) {
        ...
        this.allocateCache();
        if (!targets) this.animTargets = playable.getAnimTargets();
      }
      if (targets) this.animTargets = targets; // collection of AnimationTarget
      ...
      this.onTimer = function (/** @type {number} */ dt) {
        ...
        if (
          !self.isPlaying || // not playing
          (!self.loop && (self.curTime < self.begTime || self.curTime > self.endTime))
        ) {
          // not in range
        ...
        if (self.curTime > self.endTime) {
          ...
          for (var i = 0; i < self.animEvents.length; i++) self.animEvents[i].triggered = false;
        }
        if (self.curTime < self.begTime) self.curTime += duration;
    ...
    AnimationSession.prototype.setBlend = function (blendValue, weight, curveName) {
      if (blendValue instanceof AnimationClip) {
        if (!curveName || curveName === "") curveName = "__default__";
      ...
      else if (blendValue instanceof pc.Vec3) keyType = AnimationKeyableType.VEC;
      else if (blendValue instanceof pc.Quat) keyType = AnimationKeyableType.QUAT;
    ...
    AnimationSession.prototype.on = function (name, time, fnCallback, context, parameter) {
      if (!name || !fnCallback) return;

      var event = new AnimationEvent(name, time, fnCallback, context, parameter);
      var pos = 0;
      for (; pos < this.animEvents.length; pos++) {
        if (this.animEvents[pos].triggerTime > time) break;
      }

      if (pos >= this.animEvents.length) this.animEvents.push(event);
      else this.animEvents.splice(pos, 0, event);
    };
    ...
    AnimationSession.prototype.off = function (name, time, fnCallback) {
      ...
      for (; pos < this.animEvents.length; pos++) {
        ...
        if (!event) continue;
        if (event.name === name && event.fnCallback === fnCallback) break;

        if (event.triggerTime === time && event.fnCallback === fnCallback) break;
      }

      if (pos < this.animEvents.length) this.animEvents.splice(pos, 1);
    };
    ...
    AnimationSession.prototype.invokeByTime = function (time) {
      for (var i = 0; i < this.animEvents.length; i++) {
        if (!this.animEvents[i]) continue;

        if (this.animEvents[i].triggerTime > time) break;

        if (!this.animEvents[i].triggered) this.animEvents[i].invoke();
      }
    };
    ...
    AnimationSession.prototype.updateToTarget = function (input) {
      ...
      if (this.playable instanceof AnimationCurve && input instanceof AnimationKeyable) {
        ...
        for (j = 0; j < ctargets.length; j++) ctargets[j].updateToTarget(input.value);
        return;
      }
    ...
    AnimationSession.prototype.showAt = function (
      time,
      fadeDir,
      fadeBegTime,
      fadeEndTime,
      ...
      if (input) this._cacheKeyIdx = input._cacheKeyIdx;
      // blend related==========================================================
      ...
      for (var bname in this.blendables) {
        ...
        if (blendClip && blendClip instanceof AnimationClip && typeof p === "number") {
          var blendInput = blendClip.eval_cache(
            this.accTime % blendClip.duration,
            null,
            this._cacheBlendValues[bname]
          );
      ...
      for (var cname in this.blendables) {
        ...
        if (!blendkey || !input.curveKeyable[cname] || blendkey instanceof AnimationClip) continue;
    ...
    AnimationSession.prototype.play = function (playable, animTargets) {
      ...
      else if (animTargets) this.animTargets = animTargets;

      // reset events
      for (i = 0; i < this.animEvents.length; i++) this.animEvents[i].triggered = false;
    ...
    AnimationComponent.prototype.clipAt = function (index) {
      if (this.animClips.length <= index) return null;
    ...
    AnimationComponent.prototype.removeClip = function (name) {
      ...
      if (this.curClip === name) this.curClip = "";
    };
    ...
    AnimationComponent.prototype.setBlend = function (blendValue, weight, curveName) {
      ...
      if (curClip && curClip.session) curClip.session.setBlend(blendValue, weight, curveName);
    };
    ...
    AnimationComponent.prototype.unsetBlend = function (curveName) {
      var curClip = this.getCurrentClip();
      if (curClip && curClip.session) curClip.session.unsetBlend(curveName);
    };
    ...
    AnimationComponent.prototype.playSubstring = function (substr) {
      ...
      for (var i = 0; i < n; i++) {
        ...
        if (clip.name.indexOf(substr) !== -1) clip.play();
      }
    };
